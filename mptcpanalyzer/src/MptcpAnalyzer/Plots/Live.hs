...
startMptcpCapture :: 
      TcpConnection
   -- ^ 
   -- CaptureSettingsMptcp -> CreateProcess -> IO CaptureSettingsMptcp
...
startMptcpCapture masterFilter initialCaptureSettings createProc = do
  ...
  case exitCode of
    ...
    _ -> do
      ...
      -- hSetBuffering herr NoBuffering
      -- putStrLn $ "Live stats (before): " ++ show (lsPackets initialLiveStats)
      liveStats <- execStateT (runEffect (monitorMptcpTraffic hout)) initialCaptureSettings
      -- putStrLn $ "Live stats (after): " ++ (T.unpack . showLiveStatsTcp) liveStats
      -- putStrLn $ "Live stats (after): " ++ show (lsPackets liveStats)
      -- blocking
      exitCode2 <- waitForProcess ph
...
monitorMptcpTraffic (LivePlotTcpSettings connectionFilter mbFake mbConnectionRole ifname) = do
    ...
    trace $ "Command run: " ++ showCommandForUser bin args
    -- Log.info $ "Starting " <> tshow bin <> tshow args
    ls <- P.embed $ startMptcpCapture captureSettings createProc
    pure ls
