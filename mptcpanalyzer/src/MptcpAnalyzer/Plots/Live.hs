...
startMptcpCapture :: LiveStatsMptcp -> CreateProcess -> IO LiveStatsMptcp
startMptcpCapture initialLiveStats createProc = do
  (_, Just hout, Just herr, ph) <-  createProcess_ "error when creating process" createProc
  hSetBuffering stdout NoBuffering
  -- non blocking
  exitCode <- getProcessExitCode ph
...
{- 
-}
monitorMptcpTraffic :: Members '[Log, Cache, P.Trace, P.Embed IO] r =>
    LivePlotTcpSettings -> Sem r LiveStatsMptcp

monitorMptcpTraffic (LivePlotTcpSettings connectionFilter mbFake mbConnectionRole ifname) = do
    let
      fields = Map.elems $ Map.map tfieldFullname baseFields
    ...
    trace $ "Command run: " ++ showCommandForUser bin args
    -- Log.info $ "Starting " <> tshow bin <> tshow args
    ls <- P.embed $ startMptcpCaputre initialLiveStats createProc
    pure ls
