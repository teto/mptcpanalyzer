...
import Tshark.Main (generateCsvCommand, defaultTsharkPrefs, defaultTsharkOptions)
...
main = do
  ...
  let haskelineSettings = (Settings {
      ...
      -- TODO use generateHaskelineCompleterFromParserInfo instead
      complete = generateHaskelineCompleterFromParser defaultParserPrefs mainParser
    ...
    _ <- case specificArgs of
      ...
      (ArgsPlotLiveTcp connectionFilter mbFake connectionRole ifname) -> do
        ...
        let
          ...
          --capture-comment
          (RawCommand bin genArgs) = generateCsvCommand fields toLoad (defaultTsharkPrefs)
